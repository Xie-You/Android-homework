...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_sin:
                ...
                if (pending.length()!=0 && judge3()==0) {


                    double a = Double.parseDouble(String.valueOf(pending));
                    double angle = a * Math.PI / 180;
                    double b = sin(angle);

                    BigDecimal   c   =   new   BigDecimal(b);

                    double   result   =   c.setScale(2,   BigDecimal.ROUND_HALF_UP).doubleValue();

                    et_input.setText("sin(" + pending + ")" + "=" + result);
                }
                break;
            ...
            case R.id.btn_cos:
                ...
                et_input.setText("cos(" + pending + ")" + "=" + result);
        }
                break;
            ...
            case R.id.btn_tan:
                ...
                if (pending.length()!=0 && judge3()==0) {
                    ...
                    et_input.setText("tan(" + pending + ")" + "=" + result);
            ...
            case R.id.btn_sqrt:
                ...
                if (pending.length()!=0 && judge3()==0) {
                    ...
                    double   result   =   c.setScale(5,   BigDecimal.ROUND_HALF_UP).doubleValue();

                    et_input.setText("sqrt(" + pending + ")" + "=" + result);
                }
                break;
    ...
    private int judge3() {
        int a = 0;
        for (int i = 0; i < pending.length(); i++) {
            if (pending.charAt(i)!='0'&&pending.charAt(i)!='1'&&pending.charAt(i)!='2'&&pending.charAt(i)!='3'&&pending.charAt(i)!='4'&&pending.charAt(i)!='5'&&pending.charAt(i)!='6'&&pending.charAt(i)!='7'&&pending.charAt(i)!='8'&&pending.charAt(i)!='9') {
                a = 1;
            }
        }
        return a;
    ...
    public static class InfixInToDuffix {
        ...
        public String dealEquation(String equation){
            ...
            for (int i = 0; i < arr.length; i++) {                                    //此处就是上面说的运算过程， 因为list.remove的缘故，所以取出最后一个数个最后两个数  都是size-2
                int size = list.size();
                switch (arr[i]) {
                    case "+": double a = Double.parseDouble(list.remove(size-2))+ Double.parseDouble(list.remove(size-2)); list.add(String.valueOf(BigDecimal.valueOf(a)));     break;
                    case "-": double b = Double.parseDouble(list.remove(size-2))- Double.parseDouble(list.remove(size-2)); list.add(String.valueOf(BigDecimal.valueOf(b)));     break;
                    case "*": double c = Double.parseDouble(list.remove(size-2))* Double.parseDouble(list.remove(size-2)); list.add(String.valueOf(BigDecimal.valueOf(c)));     break;
                    //case "/": double d = Double.parseDouble(list.remove(size-2))/ Double.parseDouble(list.remove(size-2)); list.add(String.valueOf(BigDecimal.valueOf(d)));     break;
                    case "/": double d = Double.parseDouble(list.remove(size-2))/ Double.parseDouble(list.remove(size-2));
                    BigDecimal bx = new BigDecimal(d);
                    list.add(String.valueOf(bx.setScale(4, BigDecimal.ROUND_HALF_UP).doubleValue()));     break;
            ...
            return list.size() == 1 ? list.get(0) : "error" ;                    //最终list中仅有一个结果，否则就是算错了
        }
